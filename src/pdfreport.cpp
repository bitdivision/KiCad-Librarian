 ...
 *  under the License.
 *
 *  $Id: pdfreport.cpp 5030 2014-02-10 08:33:24Z thiadmer $
 */
    ...
    for (unsigned mod=0; mod<modules.Count(); mod++) {
      ...
      if (PrintPadInfo) {
        ...
        if (info.Pitch>EPSILON)
          msg=wxString::Format(wxT("Pitch: %.2f (%.1f)"),info.Pitch,info.Pitch/0.0254);
        else
          msg=wxT("Pitch: --");
                           ...
                           int pagenum,int pagecount)
{
  if (!DryRun) {
    ...
    char str[128];
    sprintf(str,"KiCad Librian %s",SVN_REVSTR);
    Text(page,PAGEMARGIN,height-PAGEMARGIN,str,0,HPDF_TALIGN_LEFT);
